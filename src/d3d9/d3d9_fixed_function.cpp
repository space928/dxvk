...
namespace dxvk {
	...
	static std::string get_file_contents(const char* filename)
	{
		...
		Logger::err(str::format("Failed to open shader file: %s with error: %d", filename, errno));
	}

	std::vector<DxvkBindingInfo> generateOverrideShaderBindings(const D3D9FFShaderKeyFS& fsKey)
	{
		...
		for (uint32_t i = 0; i < caps::TextureStageCount; i++) {
			D3DRESOURCETYPE type = D3DRESOURCETYPE(fsKey.Stages[i].Contents.Type + D3DRTYPE_TEXTURE);
	...
	void declareOverrideShaderIO(bool input, DxsoSemantic semantic, spv::BuiltIn builtin, 
		bool isVS,
		DxsoIsgn& isgn, DxsoIsgn& osgn, 
		...
		uint32_t& inputMask, uint32_t& outputMask, uint32_t& flatShadingMask)
	{
		...
		declareOverrideShaderIO(true, DxsoSemantic{ DxsoUsage::Texcoord, 3 }, spv::BuiltInMax, false, isgn, osgn, inputMask, outputMask, flatShadingMask);
		declareOverrideShaderIO(true, DxsoSemantic{ DxsoUsage::Texcoord, 4 }, spv::BuiltInMax, false, isgn, osgn, inputMask, outputMask, flatShadingMask);
		declareOverrideShaderIO(true, DxsoSemantic{ DxsoUsage::Texcoord, 5 }, spv::BuiltInMax, false, isgn, osgn, inputMask, outputMask, flatShadingMask);
		declareOverrideShaderIO(true, DxsoSemantic{ DxsoUsage::Texcoord, 6 }, spv::BuiltInMax, false, isgn, osgn, inputMask, outputMask, flatShadingMask);
		declareOverrideShaderIO(true, DxsoSemantic{ DxsoUsage::Texcoord, 7 }, spv::BuiltInMax, false, isgn, osgn, inputMask, outputMask, flatShadingMask);
		declareOverrideShaderIO(true, DxsoSemantic{ DxsoUsage::Color, 0 }, spv::BuiltInMax, false, isgn, osgn, inputMask, outputMask, flatShadingMask);
	...
	D3D9FFShader::D3D9FFShader(
		...
		if (m_d3d9Options->overrideFFShaders && (!m_d3d9Options->overrideFFShaderFilter) || Key.Stages[0].Contents.GlobalLightingEnable) {
			...
			// Setup input/output semantics
			DxsoIsgn isgn, osgn;
			uint32_t inputMask, outputMask, flatShadingMask;
			generateOverrideShaderIsgn(isgn, osgn, inputMask, outputMask, flatShadingMask);
			...
			info.outputMask = outputMask;
			info.flatShadingInputs = flatShadingMask;
			//info.pushConstOffset = m_pushConstOffset;
			//info.pushConstSize = m_pushConstSize;
			SpirvCodeBuffer spvBuffer = SpirvCodeBuffer(spv.size(), spv.data());
			m_shader = (Rc<DxvkShader>)(new DxvkShader(info, std::move(spvBuffer)));
			m_isgn = isgn;
		}
		else {
			std::string name = str::format("FF_", shaderKey.toString());
